...
## [0.16.1] - 2026-08-06
### Added
- **Определение истёкшей подписки.** Когда подписка заканчивается, провайдер продолжает отдавать фид, но подменяет все серверы заглушками — нероутируемый адрес (`0.0.0.0:1`) и имя вида «⌛ Подписка истекла» / «Продлите…». Xray при этом честно соединяется в никуда, туннель молча не отдаёт трафик, а интерфейс показывал просто «—» — неотличимо от сломанной установки. Теперь `status` возвращает поле `subscription` с кодом и человеческим текстом, а в интерфейсе появляется баннер «Подписка истекла» с сообщением провайдера, количеством доступных ручных ключей и кнопкой «Обновить подписку».
- **Причина вместо немого прочерка в «VPN IP».** `check_ips` теперь возвращает `reason`, и поле показывает, что именно не так: «подписка истекла», «Xray не запущен», «сервер недоступен» (watchdog на паузе), «сервер не отвечает». Полный текст — в подсказке при наведении.
