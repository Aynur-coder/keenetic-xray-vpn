...
### Changed
- Приоритет дедупликации: v2fly-списки авторитетны. Ручной домен-дубль убирается, кроме случая, когда у него задано отдельное подключение (исключение) — такие помечаются бейджем «исключение v2fly»
- При добавлении домена, уже покрытого v2fly-списком, без отдельного подключения — он пропускается (показывается счётчик пропущенных)
- AdGuard ipset и Xray-правила строятся по единой логике приоритетов (v2fly → исключения), без дублей
